...
utils/make_forward_fmllr.sh --single $f0dnndir $lbldir $pitchdir ""
...
rm -rf $outdir/*
utils/make_forward_fmllr.sh --single $dnndir $pitchlbldir $outdir ""
...
mkdir -p $outdir/wav/; for cmp in $outdir/cmp/*.cmp; do
    utils/mlsa_synthesis_pitch_mlpg.sh --mlpgf0done true --synth $synth --voice_thresh $voice_thresh --alpha $alpha --fftlen $fftlen --srate $srate --bndap_order $bndap_order --mcep_order $mcep_order --delta_order $delta_order $cmp $outdir/wav/`basename $cmp .cmp`.wav #$var_cmp
done

mkdir -p $outdir/wav_mlpg/; for cmp in $outdir/cmp/*.cmp; do
    utils/mlsa_synthesis_pitch_mlpg.sh --mlpgf0done true --synth $synth --voice_thresh $voice_thresh --alpha $alpha --fftlen $fftlen --srate $srate --bndap_order $bndap_order --mcep_order $mcep_order --delta_order $delta_order $cmp $outdir/wav_mlpg/`basename $cmp .cmp`.wav $var_cmp
done
